...
ms_df_full1 = pd.read_csv("/doctorai/chiarba/analysis/AT_THE_END/MS_HC_2026_h0h1.tsv" , sep="\t")
ms_df_full1.subject_number.unique()
ms_df_full.subject_number.unique()

ms_df_full[ms_df_full['subject_number']=="R22"] 
ms_df_full1[ms_df_full1['subject_number']=="R22"] 





ms_df_full[ms_df_full['cdr3_aa']=="AHGIAARSFDY"] 
ms_df_full[ms_df_full['cluster_h1']==798917]['disease_diagnosis'].unique()

ms_df_full1[ms_df_full1['cdr3_aa']=="AHGIAARSFDY"] 
ms_df_full1[ms_df_full1['cluster_h1']==798962]['disease_diagnosis'].unique()
...
#difference between ms_df_full and ms_df_full1
ms_df_full_merge = ms_df_full.merge(ms_df_full1, on=['sequence'], how='outer', indicator=True)
ms_df_full_merge
...
#load the matches with agab metadata -- debugging
res_df= pd.read_csv("/doctorai/niccoloc/MS_db/MS_BCR/aggregating_test/agab_vs_ms_lev_matches1_withAgabMetadata_IGHV.tsv", sep='\t')
ms_df.to_csv(f"/doctorai/niccoloc/MS_db/MS_BCR/ms_only1_cdr3_AGAB.csv", index=False)

ms_df['cdr3_aa'].isin(res_df['cdr3_ms']).sum()

# Optional: update MS_only1 with best match annotations
# atomic_update_ms(args.ms_only, best_for_ms, ms_key_col="cdr3_aa")
# atomic_update_ms("/doctorai/niccoloc/MS_db/MS_BCR/ms_only1_cdr3_AGAB.csv", best_for_ms, ms_key_col="cdr3_aa")


#grep for epstein
res_df_metadata_epstein = res_df_metadata[res_df_metadata['single_annotation'].str.contains("epste.*", na=False)]

# TRGAY
#look for matches of Myelin-oligodendrocyte glycoprotein
res_df_metadata[res_df_metadata['single_annotation']=="claudin"]
ms_df_full[ms_df_full['cdr3_aa']=="ARRTGFDY"] 
ms_df_full[ms_df_full['cluster_h1']==1810824]['disease_diagnosis'].unique()




res_df_metadata[res_df_metadata['single_annotation']=="Myelin-oligodendrocyte glycoprotein"]
agab_df_full_clust[agab_df_full_clust['single_annotation']=="Myelin-oligodendrocyte glycoprotein"]['cdr3_aa']
ms_df_full[ms_df_full['cdr3_aa']=="VRTGFDY"] 
ms_df_full[ms_df_full['cluster_h1']==798917]['disease_diagnosis'].unique()
ms_only_df_full[ms_only_df_full['cdr3_aa']=="AHGIAARSFDY"] 
#grep for epstein
res_df_metadata_epstein = res_df_metadata[res_df_metadata['single_annotation'].str.contains("epste.*", na=False)]
...
# antigen_to_canonical = build_antigen_to_canonical_map(res)
# df_updated = add_canonical_column(res_df, antigen_col="single_annotation", antigen_to_canonical=antigen_to_canonical)
# df_updated['single_annotation_old']=df_updated['single_annotation']
# df_updated['single_annotation']=df_updated['single_annotation_canonical']
# df_updated.to_csv("/doctorai/niccoloc/MS_db/MS_BCR/aggregating_test/agab_vs_ms_lev_matches1_withAgabMetadata_CLUSTER.tsv",sep='\t')
...
#compute fisher MS_only vs Non-MS

t0 = time.time()
rows_HC, best_for_ms_HC, checked_HC = match_all_ighv(
    agab_df, HC_df[["cdr3_aa","v_gene"]],
    max_lev=MAX_LEV,
    throttle_every=max(args.throttle_every, 0),
    sleep_sec=max(args.sleep_sec, 0.0),
    min_hits=max(args.min_hits, 1)
...
elapsed = time.time() - t0
print(f"[DONE MATCH] matches kept: {len(rows_HC)} | pairs checked (after prefilter): {checked_HC} | elapsed {elapsed/60:.1f} min")

res_df_HC = (
    pd.DataFrame(rows_HC, columns=["cdr3_agab", "cdr3_HC", "lev","vgene_match","vgene_agab","vgene_HC"])
    .sort_values(['vgene_match',"lev","cdr3_agab","cdr3_HC"])
    .reset_index(drop=True)
)

res_df_HC_metadata = res_df_HC.merge(
    agab_df_full_clust[agab_columns].drop_duplicates(),
    left_on="cdr3_agab",
    right_on="cdr3_aa",
    how="left")

res_df_HC_metadata.to_csv(args.matches_out.replace(".tsv","_withAgabMetadata_IGHV_HC.tsv"), sep="\t", index=False)
res_df_HC_metadata[res_df_HC_metadata['single_annotation']=='Myelin-oligodendrocyte glycoprotein']
res_df_HC_metadata[res_df_HC_metadata['single_annotation']=='Epstein-Barr nuclear antigen 1']
res_df_HC_metadata=pd.read_csv(args.matches_out.replace(".tsv","_withAgabMetadata_IGHV_HC.tsv"), sep="\t")

fisher_levels_HC = compute_fisher_levels_antigen(
    agab_df_full_clust,
    HC_df_OK,
    res_df_HC_metadata,
    args.out_prefix + "_HC")
...
HC_df_OK = read_cdr3(HC_df, col="cdr3_aa",vgene_col='v_gene')
HC_df_OK['is_ms']=False
HC_df_OK=HC_df_OK.merge(res_df_HC_metadata.drop('cdr3_aa', axis=1), left_on="cdr3_aa", right_on="cdr3_HC", how="left")


ms_df   = read_cdr3(ms_only_df_full, col="cdr3_aa",vgene_col='v_gene')
res_df_metadata=pd.read_csv(args.matches_out.replace(".tsv","_withAgabMetadata_IGHV.tsv"), sep="\t")
ms_df['is_ms']=True
ms_df=ms_df.merge(res_df_metadata.drop('cdr3_aa', axis=1), left_on="cdr3_aa", right_on="cdr3_ms", how="left")


full_df=pd.concat([HC_df_OK,ms_df])
#replace the Na or empty single_annotation with 'Unknown'
full_df['single_annotation'].fillna('Unknown', inplace=True)
full_df['single_annotation'].replace('', 'Unknown', inplace=True)




ms_df[ms_df['single_annotation']=='Myelin-oligodendrocyte glycoprotein']


#count how many unique single-annotations are there
res_df_metadata['single_annotation'].nunique()
ms_df['single_annotation'].nunique()
res_df_HC_metadata['single_annotation'].nunique()
HC_df_OK['single_annotation'].nunique()

full_df['single_annotation'].nunique()
...
def fisher_antigen_ms_vs_nonms(
    annotations_path: pd.DataFrame,
    out_path: str,
    antigen_col: str = "antigen",
    is_ms_col: str = "is_ms",
    padj_method: str = "fdr_bh" ):
    """
    Fisher enrichment of antigens in MS vs non-MS.
    Unit = (CDR3, antigen) annotation.
    """
    ...
    need = {antigen_col, is_ms_col}
    if not need.issubset(df.columns):
        raise ValueError(f"Missing columns {need}, found {df.columns.tolist()}")

    df = df.dropna(subset=[antigen_col, is_ms_col]).copy()
    df[is_ms_col] = df[is_ms_col].astype(bool)

    # -------------------------
    # 2) Global counts
    # -------------------------
    total_ms = df[is_ms_col].sum()
    total_nonms = (~df[is_ms_col]).sum()

    if total_ms == 0 or total_nonms == 0:
        raise ValueError("Need both MS and non-MS observations")

    # -------------------------
    # 3) Fisher per antigen
    # -------------------------
    results = []

    for antigen, sub in df.groupby(antigen_col):
        a = sub[is_ms_col].sum()
        b = (~sub[is_ms_col]).sum()
        c = total_ms - a
        d = total_nonms - b
    ...
    # -------------------------
    if not res.empty:
        res["padj"] = multipletests(res["p_value"], method=padj_method)[1]
        res = res.sort_values("padj")
    res["enriched_in"] = np.where(
        res["odds_ratio"] > 1, "MS",
        np.where(res["odds_ratio"] < 1, "non-MS", "none"))
    ...
    print(f"[DONE] Antigen MS vs non-MS Fisher written to: {out_path}")
    return res


res_test=fisher_antigen_ms_vs_nonms(
    full_df,
    args.out_prefix + "_antigen_MS_vs_nonMS.csv",
    "single_annotation",
    "is_ms",
    "fdr_bh")
