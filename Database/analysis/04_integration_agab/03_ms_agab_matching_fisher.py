...
AGAB_PATH      = "/doctorai/chiarba/AbAg_database/clean/agab_cdr3_annotated1.csv"
...
OUT_PREFIX     = "/doctorai/chiarba/AbAg_database/clean/fisher_results_annot1"
...
COLS_AGAB = dict(cdr3="cdr3_aa", antigen="antigen")
...
def enrich_matches_with_metadata(matches: pd.DataFrame,
    ...
    Only antigen is used from AgAb (no taxonomy).
    """
    m1 = matches.merge(
    ...
    m2 = m1.merge(
        agab[[COLS_AGAB["cdr3"], COLS_AGAB["antigen"]]].drop_duplicates(),
...
def compute_fisher_levels_antigen(agab_path: str, ms_path: str, matches_path: str, out_prefix: str):
    ...
    # 1b) Prep AgAb
    for col in [COLS_AGAB["cdr3"], COLS_AGAB["antigen"]]:
